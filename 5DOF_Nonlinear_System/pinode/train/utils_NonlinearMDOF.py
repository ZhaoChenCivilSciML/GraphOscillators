...
""" functions """
def Train(gnn, epochs, writer, acc, vel, dis, ag, dt, device, source_seq, target_seq):
...
def Pi_graph(dis_t_seq, vel_t_seq, ag_t, gnn, dis_mean, dis_std, vel_mean, vel_std, acc_mean, acc_std):
    ...
    gnn_input = torch.reshape(gnn_input, (gnn_input.shape[0], -1)) # (batches, states*seq*2)
    acc_GNN = de_normalize(gnn(gnn_input).unsqueeze(-1), acc_mean, acc_std) # (batches, states*seq*2) -> (batches, states) -> (batches, states, 1)
    acc_t = acc_GNN + ag_t
